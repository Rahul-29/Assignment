...
describe('AppComponent', () => {
  ...
  // get dropdown element and dispatch change event with city = London
    ...
    weatherDataMock.weather = new Array<WeatherDetails>().
    push(new WeatherDetails(mock.weather[0].id, mock.weather[0].main, mock.weather[0].description, mock.weather[0].icon));
    ...
    expect(component.averageTemperature).toEqual(weatherDataMock.main.temp);
    expect(component.windStrength).toEqual(weatherDataMock.wind.speed);
    expect(component.status).toEqual(weatherDataMock.weather.main);
    }, 1000);
